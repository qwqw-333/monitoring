...
| umz-event-detector | custom (Python) | Детектор событий регистрации UMZ → Loki (логи) |
...
Сервис `umz-event-detector` опрашивает 4 UMZ через ESL каждые 30 секунд и пушит события в Loki.
...
| event | level | Поля |
|-------|-------|------|
| `registered` | INFO | extension, ip |
| `unregistered` | WARN | extension, ip |
| `migrated` | INFO | extension, from, to, ip |
...
freeswitch-exporter/event_detector.py           — детектор событий регистрации (ESL → Loki)
