.PHONY: help up down restart logs ps status health clean pull update backup restore validate stats
...
help: ## Показать эту справку
	...
	@echo "  $(YELLOW)shell-traefik$(NC)   Shell в контейнере Traefik"
...
backup: ## Создать резервную копию volumes
	...
	@docker run --rm \
		...
		alpine:latest \
		sh -c "tar czf /backup/backup-$$(date +%Y%m%d-%H%M%S).tar.gz -C / data data2 2>/dev/null || true"
